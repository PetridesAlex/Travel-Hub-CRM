-- Employee invites join an existing agency (no new "My Travel Agency" row)
-- Run after 019. Owner invites keep existing behaviour.
...
BEGIN
  ...
  v_agency_role := lower(COALESCE(NULLIF(TRIM(NEW.raw_app_meta_data->>'agency_role'), ''), 'owner'));
